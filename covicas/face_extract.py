...
class Extract:
    def __init__(self, source=0, raspi=False, frame_drop=0, faceWidth=256, faceHeight=None,
                 flipped=True, landmarkFile="shape_predictor_68_face_landmarks.dat", channel="BGR"):
        '''
        ...
        '''
        try:
            assert isinstance(source, int) or isinstance(source, str)
        ...
        self.fa = face_utils.FaceAligner(
            predictor,
            desiredFaceWidth=faceWidth,
            desiredFaceHeight=faceHeight)
        ...
        if channel == "RGB":
            ...
            raise ValueError(
                "channel can be either of: \"RGB\"/\"BGR\",\"GRAY\"")
        # Generator Vairables
    ...
    def __next__(self):
        ...
        while self.cap.isOpened():
            ...
            if ret:
                ...
                if self.frame_drop:
                    self.frame_counter = (
                        self.frame_counter + 1) % self.frame_drop
                for rect in rects:
